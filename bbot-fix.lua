...
local menu
-- assert(getgenv().v2 == nil) throws an error because assert will check if its true and if its false/nil it will throw an error
getgenv().v2 = true
...
local placeholderImage = syn.request(
	{
		Url="https://vaxio.me/placeholderImage.txt",
		Method="GET"
	}
).Body

-- Changed from inline base64 to a http request cuz that shit is UGLY
...
elseif menu.game == "pf" then --SECTION PF BEGIN
	...
	menu.pfunload = function(self)
		...
		local gunstore = game:GetService("ReplicatedStorage").Content.ProductionContent.WeaponDatabase
		gunstore:Destroy()
		game.ReplicatedStorage:FindFirstChild(client.acchash).Name = "WeaponDatabase" -- HACK DETECTED.
	...
	local ghostchar = charcontainer.Ghosts
	...
	raspy change the GunModule directory and now i have to parse categories too! YAY!!!!

	local OLD_GUNS = game:GetService("ReplicatedStorage").Content.ProductionContent.WeaponDatabase:Clone()
	OLD_GUNS.Name = tostring(math.random(1e5, 9e5))
	client.acchash = OLD_GUNS.Name
	OLD_GUNS.Parent = game:GetService("ReplicatedStorage")

	local CUR_GUNS = game:GetService("ReplicatedStorage").Content.ProductionContent.WeaponDatabase
	]]


	local WeaponDB = game:GetService("ReplicatedStorage").Content.ProductionContent.WeaponDatabase:Clone()
	local gunModules = Instance.new("Folder")

	for i,v in next, WeaponDB:GetChildren() do
	...
	local OLD_GUNS = gunModules:Clone()

	OLD_GUNS.Name = tostring(math.random(1e5, 9e5))
	client.acchash = OLD_GUNS.Name
	OLD_GUNS.Parent = game:GetService("ReplicatedStorage")
	local CUR_GUNS = gunModules
					...
					clienteventfuncs[hash] = function(gun, mag, spare, attachdata, camodata, gunn, ggequip)
						...
						if client.fakecharacter then
							client.fakeupdater.equip(
								-- TODO: fix getting gun category, guns, gun data
							)
						end
					end
				end
